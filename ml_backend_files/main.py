...
def run_pipeline():
    ...
    # Skala Rupiah: harga per baris = adr(EUR) x kurs = (ratio x BASE_REFERENCE) x kurs.
    # Base rupiah konstan agar rupiah = Price Ratio x (BASE_REFERENCE x kurs).
    base_rupiah = BASE_REFERENCE * EXCHANGE_RATE_EUR_IDR
    base_prices = np.full(len(df), base_rupiah, dtype=float)
